...
export async function getListings(
  ...
  try {
    ...
    // Build the OData filter from search params
    ...
    // Sorting — default to newest modifications first
    queryParams['$orderby'] = params.sort || 'ModificationTimestamp desc';

    return await bridgeFetch<Listing>('/listings', queryParams);
  } catch (error) {
    // Gracefully degrade — return empty results instead of crashing the page
...
export async function getListing(id: string): Promise<Listing | null> {
  try {
    ...
    // Return null instead of crashing — the page can show a 404
    return null;
  }
}
